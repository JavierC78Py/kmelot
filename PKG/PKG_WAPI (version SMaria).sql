PROMPT CREATE OR REPLACE PACKAGE clinicasm.pkg_wapi
CREATE OR REPLACE package clinicasm.pkg_wapi as
    c_modulo constant varchar2(20) := 'BAILEYS';
    g_url    constant varchar2(255) := 'https://kmelot.online/baileys02';
    g_port   constant number        := null;
    g_key    constant varchar2(255) := '0a580dfc-120b-458a-b6c8-5c288a48ea4f';
  ...
  FUNCTION start_session(P_SESSION IN varchar2) return clob;

  FUNCTION qr_b64(p_key in varchar2) return clob;

  FUNCTION check_instance(p_key in varchar2) return number;

  FUNCTION check_session(P_KEY IN VARCHAR2) RETURN NUMBER;
  ...
  FUNCTION SEND_TEXT ( P_KEY      IN VARCHAR2
  ...
  FUNCTION logout_instance(P_INSTANCE IN VARCHAR2) RETURN VARCHAR2;
...
PROMPT CREATE OR REPLACE PACKAGE BODY clinicasm.pkg_wapi
CREATE OR REPLACE PACKAGE BODY clinicasm.pkg_wapi AS
  --==============================================================================
  /*
  ...
  */
  --==============================================================================
    ...
    BEGIN
        ...
        DBMS_LOB.CONVERTTOCLOB(NCLOB, PBLOB, NFILE_SIZE, NDEST_OFFSET, NSRC_OFFSET,
                               NBLOB_CSID, NLANG_CONTEXT, NWARNING);
  ...
  --COMMIT;
    ...
    THEN
      ...
      */
    END IF;
    IF l_type NOT IN ('messages.upsert', 'send.message') OR l_remote_jid = 'status@broadcast' THEN
      RETURN;
    END IF;
    IF l_remote_jid LIKE '%@g.us' THEN
      RETURN;
    ELSE
    ...
    IF l_message IS NULL THEN
      l_message := apex_json.get_varchar2(
         P_PATH => 'data.result.message.extendedTextMessage.text'
        ,P_VALUES => l_json_values);
    END IF;
    IF l_message IS NULL THEN
      l_message := apex_json.get_varchar2(
    ...
    IF l_message IS NULL THEN
      l_message := apex_json.get_varchar2(
         P_PATH => 'data.messages.message.ephemeralMessage.message.extendedTextMessage.text'
        ,P_VALUES => l_json_values);
    END IF;
    ...
    IF v_chat_id IS NOT NULL THEN
      ...
      BEGIN
        l_message2 := l_message;
        DECLARE
          v_err_guardar VARCHAR2(1000);
        BEGIN
          IF l_message2 IS NULL THEN
            l_response2 := l_response;
          END IF;
          ...
          INSERT INTO chats_details (
             ...
             ,current_timestamp - INTERVAL '3' HOUR      --created
             ,l_fromme                     --owner
             ,SubStr(l_pushname,1,50)      --username
             ,l_id                         --message_id
             ,l_response2                  --json
             ,l_latitude                   --latitude
        ...
        EXCEPTION
          WHEN OTHERS THEN
            v_err_guardar := SQLERRM;
            INSERT INTO tmp_javier VALUES (SYSDATE,v_err_guardar);
            COMMIT;
        END;
...
--          INSERT INTO tmp_javier VALUES (SYSDATE,v_obtener);
--          COMMIT;
  ...
  END web_socket;
    ...
    PROCEDURE SEND_BULK_MESSAGE IS
        ...
        L_FROMME        NUMBER := 0;
        L_CHATS         CHATS%ROWTYPE;
        TYPE T_CHATS_DETAILS IS
            TABLE OF CHATS_DETAILS%ROWTYPE INDEX BY BINARY_INTEGER;
        L_CHATS_DETAILS T_CHATS_DETAILS;
        L_TYPE          VARCHAR2(25);
        V_DEVICE_ID     NUMBER;
        V_CHAT_ID       NUMBER;
        ...
        SELECT
            C.ID_COLA_MENSAJE ID,
            REGEXP_REPLACE(
                REPLACE(
                    REPLACE(C.DESTINATARIO, ' ', ''),
                    '+',
                    NULL
                ),
                '^(\+?595|\d{2})0?(\d{1})(\d{6})$',
            ...
            C.FECHA_ALTA
                 ...
                 -- ,c.enviar
            ,
            C.FECHA_ENVIAR    FECHA_PROGRAMADO,
            NULL              BLOB_CONTENT,
            NULL              MIME_TYPE,
            NULL              FILENAME,
            C.CUENTA_ENVIAR   CUENTA,
            C.FECHA_ENVIADO
        FROM
            COLA_MENSAJE C
        WHERE
            C.FECHA_ENVIADO IS NULL
            AND C.ERROR IS NULL
            AND ( ( C.FECHA_ENVIAR IS NULL
                    AND C.FECHA_ALTA >= SYSDATE - 1 )
                  OR C.FECHA_ENVIAR <= SYSDATE )
            AND ( ( C.CUENTA_ENVIAR = P_CUENTA )
                  OR ( C.CUENTA_ENVIAR IS NULL /*AND EXISTS
                   (
                     SELECT 1
                     FROM parametros p
                     WHERE p.explicacion = 'NUMEROS WHATSAPP HABILITADO SIN CUENTA'
                     AND p.descripcion = p_cuenta
                   )*/ ) )
        ORDER BY
            C.PRIORIDAD DESC,
            C.DESTINATARIO ASC,
        ...
        FROM
            DEVICES
        WHERE
            PKG_WAPI.CHECK_INSTANCE(KEY) = 1;

    BEGIN
        FOR I IN C_INSTANCIAS LOOP
            FOR C IN C_COLA(I.PHONE_NUMBER) LOOP
                BEGIN
                    IF
                        C.BLOB_CONTENT IS NULL
                        AND C.MIME_TYPE IS NULL
                    THEN
                        V_MESSAGE_ID := SEND_TEXT(
                            P_KEY      => I.KEY,
                            P_NUMBER   => C.DESTINATARIO,
                            P_MESSAGE  => C.MENSAJE,
                            P_RESPONSE => L_ERROR
                        );
                    ELSE
                        V_MESSAGE_ID := SEND_FILE_URL(
                            P_KEY      => I.KEY,
                            P_NUMBER   => C.DESTINATARIO,
                            P_URL      => 'http://192.168.41.31:8285/ords/santaclara/cola_mensajes/content/' || C.ID,
                            P_TYPE     =>
                                    CASE
                                                  WHEN INSTR(C.MIME_TYPE, 'pdf') > 0 THEN
                                                      'document'
                                                  WHEN INSTR(C.MIME_TYPE, 'jpg') > 0
                                                       OR INSTR(C.MIME_TYPE, 'jpeg') > 0
                                                       OR INSTR(C.MIME_TYPE, 'png') > 0 THEN
                                                      'image'
                                                  ELSE
                                                      'unknown'
                                    END,
                            P_MIMETYPE => C.MIME_TYPE,
                            P_MESSAGE  => C.MENSAJE,
                            P_RESPONSE => L_ERROR
                        );
                    END IF;
                    ...
                    IF V_MESSAGE_ID IS NOT NULL THEN
                        UPDATE COLA_MENSAJE CM
                        SET
                            CM.FECHA_ENVIADO = SYSDATE,
                            CM.CUENTA_ENVIO = I.PHONE_NUMBER
                        WHERE
                            CM.ID_COLA_MENSAJE = C.ID;

                    ELSE
                        UPDATE COLA_MENSAJE CM
                        SET
                            CM.FECHA_ERROR = SYSDATE,
                            CM.ERROR = L_ERROR
                        WHERE
                            CM.ID_COLA_MENSAJE = C.ID;
                    ...
                    L_ENVIADO := L_ENVIADO + SQL%ROWCOUNT;
                    LOGGER('Destinatario: '
                           || C.DESTINATARIO
                           || CHR(10)
                           || 'Mensaje: '
                ...
                EXCEPTION
                    -- Capturar cualquier excepción que ocurra dentro del bloque BEGIN...END
                    WHEN OTHERS THEN
                        -- Imprimir el error o realizar cualquier acción de manejo de errores necesaria
                        LOGGER('Error to send bulk message. : '
                               || CHR(10)
                               || 'Error: '
                               || SQLERRM
                               || CHR(10)
                               ...
                               'PKG_WAPI.SEND_BULK_MESSAGE');
                        --Continuar con la iteración del bucle
                        CONTINUE;
                END;
            END LOOP;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            RAISE_APPLICATION_ERROR(-20099,
                                    'ERROR AL EJECUTAR EL SEND BULK MESSAGES: '
                                    || SQLERRM
                                    || CHR(10)
                                    || 'Trace: '
                                    || DBMS_UTILITY.FORMAT_ERROR_BACKTRACE());

        --logger('Error to send bulk message. : ' || chr(10) ||
        --       'Error: ' || sqlerrm || chr(10) || 'Trace: ' ||
        --       dbms_utility.format_error_backtrace(),
        --       'PKG_WAPI.SEND_BULK_MESSAGE');
    END;
  ...
  END send_immediate_text_message;
    ...
    PROCEDURE SEND_NEW_TEXT_MESSAGE (
        P_REMOTE_JID IN CHATS.REMOTE_JID%TYPE
       ,P_MENSAJE    IN VARCHAR2
       ,P_ID_DEVICE  IN DEVICES.ID%TYPE
    ) IS
      v_destinatario CHATS.REMOTE_JID%TYPE;
    ...
    BEGIN
    /*
    Enviar mensaje a nuevo destinatario.
    JavierC  28-05-2024 Creado
    */
      SELECT d.id
            ,d.key
      INTO v_device_id
      ...
      BEGIN
        ...
        INTO v_destinatario, v_chat_id
        FROM chats c
        WHERE c.remote_jid = p_remote_jid
      ...
      EXCEPTION
        WHEN No_Data_Found THEN
          ...
          INSERT INTO chats (
           ...
           ,NULL              --modified
           ,NULL              --push_name
           ,P_REMOTE_JID      --remote_jid
           ,1                 --read
          );
      END;
      v_message_id := send_text(
            P_KEY      => v_key,
            P_NUMBER   => P_REMOTE_JID,
            P_MESSAGE  => P_MENSAJE,
            P_RESPONSE => l_error);
    --Inserta el mensaje por el web socket
    --IF v_message_id IS NOT NULL THEN
    --  INSERT INTO chats_details (
    --    CHAT_ID,
    --    MESSAGE,
    ...
    --  ) VALUES (
    --    v_chat_id                --chat_id
    --   ,TO_CLOB(P_MENSAJE)       --message
    --   ,current_timestamp        --created
    --   ,'S'                      --owner
    --   ,Nvl(V('APP_USER'),USER)  --username
  ...
  BEGIN
    BEGIN
      ...
      ;
      Dbms_Output.Put_Line('pasamos:'||PI_CHAT_ID);
      ...
      WHERE d.es_bot = 1
      ;
      Dbms_Output.Put_Line('pasamos2');
    EXCEPTION
      WHEN No_Data_Found THEN
        --Raise_Application_Error(-20999, 'Error en la busqueda del chat id:' || PI_CHAT_ID);
        RETURN;
    END;
    FOR msg IN (
      SELECT
            ...
            IF Upper(msg.capture_option) = Upper(SubStr(PI_MENSAJE,1,v_ancho_capture)) THEN
              ...
              IF msg.answer_response IS NOT NULL THEN
                v_message_id := send_text(
                      P_KEY      => v_key
                     ,P_NUMBER   => v_remote_jid
                     ,P_MESSAGE  => REPLACE(msg.answer_response,'#SALTO#',EOL)
                     ,P_RESPONSE => l_error
                );
                IF v_message_id IS NOT NULL THEN
                  BEGIN
  ...
  END P_BOT;
    ...
    BEGIN
        L_RESPONSE := APEX_WEB_SERVICE.MAKE_REST_REQUEST(
            P_URL         => G_URL
                     || '/group/getallgroups?key='
                     || PI_KEY,
            P_HTTP_METHOD => 'GET'
        );

        APEX_JSON.PARSE(L_RESPONSE);
      --SELECT json_serialize(dato_json returning VARCHAR2 pretty)
      --INTO l_response
      --FROM test_json;
        ...
        FOR I IN (
            SELECT
                ...
                JT.SUBJECT_VALUE
            FROM
                    JSON_TABLE ( L_RESPONSE, '$.instance_data.*'
                        COLUMNS (
            ...
            UPDATE CHATS C
            SET
                C.PUSH_NAME = I.SUBJECT_VALUE
            WHERE
                I.ID_VALUE LIKE C.REMOTE_JID || '%';
...
--                  ,V_JSON_BOT             --json
                ...
                UPDATE SM_RESERVAS S
                SET S.TELEFONO_ENVIADO = V_PHONE_NUMBER,
                    S.FECHA_ENVIADO = ( CURRENT_DATE - INTERVAL '3' HOUR )
                WHERE S.ID_REGISTRO = I.ID_REGISTRO;
                COMMIT;
            END IF;
  ...
  BEGIN
    ...
    v_url := 'https://gc42a911feb841d-dbkmelot.adb.sa-santiago-1.oraclecloudapps.com/ords/clinicasm/wha_img/imagen/' || PI_ID_ARCHIVO;
    v_tipo := CASE
  ...
  EXCEPTION
    WHEN OTHERS THEN
      pkg_wapi.logger('ERROR: ' || SQLERRM, 'P_ENVIAR_ARCHIVO');
