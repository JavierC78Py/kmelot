PROMPT CREATE OR REPLACE VIEW salotex.v_movimiento_producto_deposito
CREATE OR REPLACE VIEW salotex.v_movimiento_producto_deposito (
  tipo,
  fecha_evento,
  id_transaccion,
...
SELECT v.tipo
      ...
      ,v.nombre_entidad
      ,v.orden
      ,sum(v.cantidad) OVER (PARTITION BY v.pro_id_producto, v.dep_id_deposito ORDER BY v.pro_id_producto, v.dep_id_deposito, v.orden) stock
FROM (
  SELECT b.tipo
        ,b.fecha_evento
        ,b.id_transaccion
  ...
  FROM (
    ...
    AND e.id_entidad = c.ent_id_entidad
    ORDER BY pro_id_producto, dep_id_deposito, fecha_evento
  ) b
) v
/
