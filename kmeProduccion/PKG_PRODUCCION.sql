PROMPT
CREATE OR REPLACE PACKAGE salotex.pkg_produccion
...
/*
 * Procedimiento para cargar los insumos (Materia Prima y Operativos)
 * basandose en la Receta y la cantidad a producir de la Orden.
 */
PROCEDURE cargar_insumos_orden (p_id_orden_prod IN NUMBER);
END pkg_produccion;
/ PROMPT
CREATE OR REPLACE PACKAGE BODY salotex.pkg_produccion
CREATE OR REPLACE PACKAGE BODY salotex.pkg_produccion AS PROCEDURE finalizar_orden (
    p_id_orden_prod IN NUMBER,
    p_usuario IN VARCHAR2 DEFAULT NVL(v('APP_USER'), USER)
  ) IS v_estado_actual VARCHAR2(20);
v_id_ajuste NUMBER;
v_sucursal NUMBER;
...
-- Asumiendo 1 como ID de Moneda Base (Guaraníes/Pesos)
v_tipo_cambio NUMBER := 1;
v_unidad_medida_fin NUMBER;
BEGIN -- 1. Validar Estado de la Orden
SELECT estado,
  suc_id_sucursal,
  dep_id_deposito_orig,
  dep_id_deposito_dest,
  pro_id_producto_fin,
  cantidad_a_producir INTO v_estado_actual,
  v_sucursal,
  v_deposito_orig,
  v_deposito_dest,
  v_producto_fin,
...
IF v_estado_actual = 'FINALIZADO' THEN RAISE_APPLICATION_ERROR(
  -20001,
  'La orden de producción ya se encuentra FINALIZADA.'
);
ELSIF v_estado_actual = 'ANULADO' THEN RAISE_APPLICATION_ERROR(
  -20002,
  'No se puede finalizar una orden ANULADA.'
);
END IF;
-- Obtener la unidad de medida del producto final desde la receta usada
SELECT r.uni_id_unidad_medida INTO v_unidad_medida_fin
...
VALUES (
    ...
    v_tipo_cambio,
    0,
    -- El Trigger se encarga de sumar los montos del detalle
...
) LOOP v_costo_total_orden := v_costo_total_orden + NVL(c_mat.costo_total, 0);
...
FOR c_ope IN (
  ...
  WHERE orp_id_orden_prod = p_id_orden_prod
) LOOP v_costo_total_orden := v_costo_total_orden + NVL(c_ope.costo_real_total, 0);
END LOOP;
...
VALUES (
    ...
    v_unidad_medida_fin,
    CASE
      WHEN v_cantidad_producir > 0 THEN v_costo_total_orden / v_cantidad_producir
      ELSE 0
    END,
    v_cantidad_producir,
    -- POSITIVO para sumar stock
    v_costo_total_orden,
    v_deposito_dest
  );
...
WHERE id_orden_prod = p_id_orden_prod;
EXCEPTION
WHEN OTHERS THEN -- Permite que el error suba a APEX (o al entorno de ejecucion) para hacer ROLLBACK
RAISE;
END finalizar_orden;
PROCEDURE cargar_insumos_orden (p_id_orden_prod IN NUMBER) IS v_receta NUMBER;
v_cantidad_a_producir NUMBER;
v_cantidad_base NUMBER;
v_count_insumos NUMBER;
BEGIN -- 1. Obtener la Receta y la Cantidad de la Orden
SELECT o.rec_id_receta,
  o.cantidad_a_producir,
  nvl(r.cantidad_base, 1) INTO v_receta,
  v_cantidad_a_producir,
  v_cantidad_base
FROM cab_orden_produccion o
  JOIN cab_receta_produccion r ON o.rec_id_receta = r.id_receta
WHERE o.id_orden_prod = p_id_orden_prod;
-- Validar si ya tiene insumos cargados para evitar duplicados
SELECT COUNT(*) INTO v_count_insumos
FROM det_orden_produccion_mat
WHERE orp_id_orden_prod = p_id_orden_prod;
IF v_count_insumos > 0 THEN RAISE_APPLICATION_ERROR(
  -20003,
  'La orden ya tiene insumos cargados. Por favor borre los detalles actuales de la grilla antes de volver a cargarlos desde la receta.'
);
END IF;
-- 2. Insertar los insumos (Materia Prima)
...
-- 3. Insertar Costos Operativos de la Receta
